...
)
{
     ...
     * */;
    double C_0  = (1.0/256.0)*(63)      ;
    double C_2  = (1.0/256.0)*(3465.0)  ;
    double C_4  = (1.0/256.0)*(30030.0) ;
    double C_6  = (1.0/256.0)*(90090.0) ;
    double C_8  = (1.0/256.0)*(109395.0);
    double C_10 = (1.0/256.0)*(46189.0) ;
    double x_2, x_4, x_6, x_8, x_10;
...
)
{
    ...
    double C_1  = (1/128.0)*(315.0)  ;
    double C_3  = (1/128.0)*(4620.0) ;
    double C_5  = (1/128.0)*(18018.0);
    double C_7  = (1/128.0)*(25740.0);
    double C_9  = (1/128.0)*(12155.0);
    double x_2, x_3, x_5, x_7, x_9;
...
static void test_Gauss_points(void)
{
    ...
    matlib_dv gzeros, y; /* must be matlib_freed */ 
    matlib_create_dv(p, &gzeros, MATLIB_COL_VECT);
    matlib_create_dv(p, &y, MATLIB_COL_VECT);
...
static void test_LGL_points(void)
{
    ...
    matlib_dv gzeros, zeros, quadW, y, yy; /* must be matlib_freed */

    matlib_create_dv(     p, &gzeros, MATLIB_COL_VECT);
    matlib_create_dv(nr_LGL,  &zeros, MATLIB_COL_VECT);
    matlib_create_dv(nr_LGL,  &quadW, MATLIB_COL_VECT);
    matlib_create_dv(nr_LGL,      &y, MATLIB_COL_VECT);
    matlib_create_dv(nr_LGL,     &yy, MATLIB_COL_VECT);
    ...
    /* Verification */ 
    double *ptr, *ptr1 = zeros.elem_p, *ptr2 = yy.elem_p ;
    for(ptr= y.elem_p; ptr < y.elem_p+y.len; ptr++)
    {
...
static void test_forward_transform(void)
{
     ...
     * */
    double U_2[] = {   1.0/3,       0,     2.0/3                                      };
    double U_3[] = {       0,   3.0/5,         0,   2.0/5                             };
    double U_4[] = {  7.0/35,       0,   20.0/35,       0,   8.0/35                   };
    double U_5[] = {       0, 27.0/63,         0, 28.0/63,        0, 8.0/63           };
    double U_6[] = {33.0/231,       0, 110.0/231,       0, 72.0/231,      0, 16.0/231 };
    
    int i, j, nr_MLT = 5, p, nr_LGL;
    /* An array of pointers */ 
    double *MLT_array[5] = {U_2, U_3, U_4, U_5, U_6};
    
    matlib_dv xi, quadW, u, U, U_exact;
    matlib_dm FM;
    double e_relative, norm_exact;
    ...
    for (i=0; i<nr_MLT; i++)
    {
        ...
        nr_LGL = p+1;
        matlib_create_dv( nr_LGL, &u,       MATLIB_COL_VECT);
        matlib_create_dv( nr_LGL, &U,       MATLIB_COL_VECT);
        matlib_create_dv( nr_LGL, &U_exact, MATLIB_COL_VECT);
        
        monomial_func(p, xi, u);
        /* U: LT of u */ 
        matlib_dgemv( 1.0, FM, u, 0, U);
        U_exact.len    = nr_LGL;
        ...
        norm_exact = matlib_dnrm2(U_exact);
        matlib_daxpy(-1.0, U_exact, U);
        e_relative = matlib_dnrm2(U)/norm_exact;
...
static double test_legendre_interpolation_general
(
    matlib_index p,
...
)
{
    ...
    DEBUG_PRINT_DV(U, "%s: ", "transfomred matrix");

    /* Verification =========================*/
    matlib_index P = nr_samples - 1;

    matlib_dm IMi;
    matlib_dv xii, u_interpol, u_actual;

    matlib_create_dv( nr_samples, &xii,        MATLIB_COL_VECT);
    matlib_create_dv( nr_samples, &u_interpol, MATLIB_COL_VECT);
    matlib_create_dv( nr_samples, &u_actual,   MATLIB_COL_VECT);
    ...
    *(xii.elem_p+P) = 1.0;
    ...
    matlib_dgemv( 1.0, IMi, U, 0, u_interpol);
...
static void test_FM_and_IM(void)
{
    ...
    matlib_dm FM, IM, FMi, IMi;
    matlib_dv xi, quadW, u, U, Ui;

    double e_relative, norm_U;
    ...
    for (i=0; i<ni; i++)
    {
        ...
        matlib_create_dv( xi.len, &u, MATLIB_COL_VECT);
        matlib_create_dv( P+1,    &U, MATLIB_COL_VECT);
        
        Gaussian_func(xi, u);
        /* U: LT of u */ 
        matlib_dgemv( 1.0, FM, u, 0, U);

        matlib_create_dm( p+1, xi.len, &FMi, MATLIB_ROW_MAJOR, MATLIB_NO_TRANS);
        legendre_LGLdataFM( xi, FMi);

        matlib_create_dv( p+1,    &Ui, MATLIB_COL_VECT);

        /* U: LT of u */ 
        matlib_dgemv( 1.0, FMi, u, 0, Ui);
        U.len = Ui.len;
        norm_U = matlib_dnrm2(U);
        matlib_daxpy(-1.0, U, Ui);
        e_relative = matlib_dnrm2(Ui)/norm_U;
        debug_body("relative error: % 0.16g", e_relative);
        CU_ASSERT_TRUE(e_relative==0);

        matlib_free( (void*) FM.elem_p);
        matlib_free( (void*) FMi.elem_p);
        matlib_free( (void*) u.elem_p);
        matlib_free( (void*) U.elem_p);
        matlib_free( (void*) Ui.elem_p);
        matlib_free( (void*) quadW.elem_p);
        matlib_free( (void*) xi.elem_p);
    }
    debug_exit("%s", "");
...
static void test_parsevals_theorem(void)
{
    ...
    matlib_dm FM, IM;
    matlib_dv xi, quadW, u, U, coeff;
    
    double e_relative[ni], snorm_spectral, snorm_actual = sqrt(M_PI/32.0);
    ...
    for (i=0; i<ni; i++)
    {
        ...
        matlib_create_dv( xi.len,     &u, MATLIB_COL_VECT);
        matlib_create_dv(    p+1,     &U, MATLIB_COL_VECT);
        matlib_create_dv(    p+1, &coeff, MATLIB_COL_VECT);
        ...
        snorm_spectral = matlib_ddot( U, coeff);
