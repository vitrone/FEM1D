...
void test_pfem1d_solve_Schroedinger_IVP2c(void)
{
    matlib_index p = 4;
    matlib_index nr_LGL = 2*p+1;
    matlib_index N  = 4000;
...
void* _thfunc_assemble_GSMM(void* mp)
{
    debug_enter("%s", "");
    pthpool_arg_t *ptr = (pthpool_arg_t*) mp;
    debug_body("%s", "pthpool_arg_t *ptr initialized");
...
void* _thfunc_evolve_field(void* mp)
{
    ...
    matlib_zv V_tmp   = *((matlib_zv*) (ptr->shared_data[16]));
    matlib_zv u_exact = *((matlib_zv*) (ptr->shared_data[17]));
    matlib_zv Pvb     = *((matlib_zv*) (ptr->shared_data[18]));
    matlib_zv V_vb    = *((matlib_zv*) (ptr->shared_data[19]));
    matlib_xm FM      = *((matlib_xm*) (ptr->shared_data[20]));

    pardiso_solver_t* data = ((pardiso_solver_t*) (ptr->shared_data[21]));
    matlib_xv* e_relative  = ((matlib_xv*) (ptr->shared_data[22]));
    ...
    if(switch_sparse)
    {
        data->smat_p = (ptr->shared_data[11]);
    }
    else
    {
        data->smat_p = (ptr->shared_data[10]);
    }
    matlib_real norm_actual;

    debug_body( "nsparse: %d", ((matlib_zm_nsparse*)data->smat_p)->nsparse);
    for( matlib_index j=0; j<((matlib_zm_nsparse*)data->smat_p)->nsparse; j++)
    {
        fem1d_ZPrjL2F(p, U_tmp, Pvb);
        ...
        debug_body("norm actual: % 0.16g", norm_actual);
        matlib_zaxpy(-1.0, U_tmp, V_tmp );
        e_relative->elem_p[j] = fem1d_ZNorm2(p, N, V_tmp)/norm_actual;
        debug_body("Relative error: % 0.16g", e_relative->elem_p[j]);
    }
    debug_exit("%s", "");
}
...
)
{
    ...
    matlib_index N_ = N, p_ = p;
    matlib_index switch_sparse = 0;
    matlib_index num_threads_ = num_threads-2;
    ...
    arg[0].shared_data = shared_data;
    arg[0].thread_index = 0;
    task[0].function = (void*) _thfunc_assemble_GSMM;
    task[0].argument  = &arg[0];

    arg[1].shared_data = shared_data;
    arg[1].thread_index = 1;
    task[1].function = (void*) _thfunc_evolve_field;
    task[1].argument  = &arg[1];
    ...
    pthpool_exec_task(1, &mp[1], &task[1]);
...
int main(void)
{
    ...
    CU_TestInfo test_array[] = 
    {
        ...
        //{ "Linear time-dependent potential" , test_pfem1d_solve_Schroedinger_IVP2c},
