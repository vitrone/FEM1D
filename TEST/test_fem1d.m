classdef test_fem1d < matlab.unittest.TestCase
% test = test_fem1d
% test.run
% run(test, 'test_GP_ABC1a_CQ(testCase')
    ...
    methods(Test)
        function test_interpolation(testCase)
            ...
            xi = (-1:0.001:1)';
            P  = length(xi)-1;
            IM1 = LGLdataIM( P, p, xi);
            u1  = fem_ilt( p, N, P, IM1, U);
        ...
        function test_interpolation2(testCase)
            ...
            u = poly_fun(x);      % a column vector
            U = fem_flt2( p, N, p, FM, u, 3);

            xi = (-1:0.001:1)';
            P  = length(xi)-1;
            IM1 = LGLdataIM( P, p, xi);
            u1  = fem_ilt2( p, N, P, IM1, U, 3);
            x1  = ref2mesh (xi, N, [x_l x_r]);
            u_actual = poly_fun(x1);
        ...
        function test_norm(testCase)
            ...
            U = fem_flt( p, N, p, FM, u);
            lp_norm = sqrt(J)*femlp_norm2(p, N, U);
