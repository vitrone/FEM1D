classdef xUnit_legendre < matlab.unittest.TestCase
% test = xUnit_legendre
% run(test)
    ...
    methods(Test)
        ...
        function test_interpolation(testCase)
            ...
            poly_fun = @(x) x.^p;
            tol = 1e-9;
            [xir, quadW, IM, FM] = mxLGLdataLT2(2*p, tol);
            F = poly_fun(xir);      % a column vector
            U = FM*F;               % discrete Legendre transform

            xi = (-1:0.001:1)';
            P = length(xi)-1;
            IM1 = mxLGLdataIM( 2*p, xi);
